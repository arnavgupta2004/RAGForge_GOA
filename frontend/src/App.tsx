...
function App() {
  ...
  return <main className="app-shell"><header><a className="brand" href="#top"><span className="brand-mark">R</span><span>RAG<span>Forge</span></span></a><div className="header-copy"><span>ADAPTIVE VOICE RAG</span><i /><span className="hh-context">HH GOA '26 · #RAGInGoa</span></div><div className={`system-status ${health?.status === "ok" ? "online" : ""}`}><i />{health?.status === "ok" ? health.chunks_indexed > 0 ? "API ONLINE · INDEX READY" : "SYSTEM ONLINE" : "CONNECTING"}</div></header>
    ...
    <section className="command-center"><div className={`voice-side ${busy || recorder.status === "recording" ? "processing" : ""} ${result?.status === "refused" ? "blocked" : ""}`}><div className={`waveform ${recorder.status === "recording" ? "live" : ""}`} aria-hidden="true">{Array.from({ length: 15 }, (_, i) => <i style={{ height: `${16 + (recorder.level || 0.15) * (i % 5 + 3) * 10}px` }} key={i} />)}</div><button className={`mic-button ${recorder.status === "recording" ? "recording" : ""}`} onClick={onMic} disabled={busy && recorder.status !== "recording"} aria-label={recorder.status === "recording" ? "Stop recording" : "Start voice recording"}><span>{recorder.status === "recording" ? "■" : "◉"}</span></button><p className="voice-status">{statusText}</p><p className="voice-helper">English + Hindi voice input via Sarvam</p></div>
